...
export default function Results() {
  ...
  const getDisplayReviews = () => {
    ...
    if (isDad && viewAsYouth) {
      // Dad viewing as youth - show only reviews about themselves
      return selfMember ? reviews.filter(r => r.memberId === selfMember.id) : [];
    } else if (isDad) {
      // Dad in normal view - see ALL reviews
      return reviews;
    } else {
          ...
          <div className="flex gap-3 items-center flex-wrap">
            {isDad && (
              <div className="flex items-center gap-2 mr-2">
                <button
                  onClick={() => setViewAsYouth(false)}
                  className={`px-3 py-2 rounded-lg font-semibold text-sm flex items-center gap-1 ${!viewAsYouth ? "text-white" : ""}`}
                  style={{ background: !viewAsYouth ? AZ_BLUE : "#e5e7eb", color: !viewAsYouth ? "white" : AZ_BLUE }}
                >
                  <Users className="w-4 h-4" />
                  All
                </button>
                <button
                  onClick={() => setViewAsYouth(true)}
                  className={`px-3 py-2 rounded-lg font-semibold text-sm flex items-center gap-1 ${viewAsYouth ? "text-white" : ""}`}
                  style={{ background: viewAsYouth ? AZ_BLUE : "#e5e7eb", color: viewAsYouth ? "white" : AZ_BLUE }}
                >
            ...
            {isDad && !viewAsYouth && <button onClick={exportCSV} className="px-4 py-2 rounded-lg font-semibold text-white flex items-center gap-2" style={{ background: AZ_BLUE }}><FileDown className="w-4 h-4" />Export CSV</button>}
        ...
        {averages.length === 0 ? (
          ...
          <>
            <h2 className="text-xl font-semibold mb-4" style={{ color: AZ_BLUE }}>
              {isDad && viewAsYouth ? "Reviews About You (Youth View)" : isDad ? "Average Ratings by Member (All Reviews)" : isYouth ? `Reviews for ${selfMember?.name || "You"}` : "Average Ratings by Member"}
            </h2>
