...
export default function Results() {
  ...
  const getDisplayReviews = () => {
    ...
    if (isDad && testMode) {
      // In test mode, show only Seth Baldwin's reviews (SMC view simulation)
      return reviews.filter(r => r.memberId === "seth-baldwin");
    } else if (isDad) {
      ...
      <main className="max-w-6xl mx-auto px-4 py-8">
        <div className="mb-6 flex justify-between items-center flex-wrap gap-4">
          ...
          <div className="flex gap-3 items-center flex-wrap">
            {isDad && (
              <>
                <button onClick={() => setTestMode(!testMode)} className="px-4 py-2 rounded-lg font-semibold text-white" style={{ background: testMode ? AZ_RED : AZ_GOLD, color: testMode ? "white" : AZ_BLUE }}>
                  {testMode ? "Reset to Dad View" : "View as SMC"}
                </button>
                <button onClick={exportCSV} className="px-4 py-2 rounded-lg font-semibold text-white flex items-center gap-2" style={{ background: AZ_BLUE }}><FileDown className="w-4 h-4" />Export CSV</button>
              </>
            )}
        ...
        {averages.length === 0 ? (
          ...
          <>
            <h2 className="text-xl font-semibold mb-4" style={{ color: AZ_BLUE }}>
              {isDad && testMode ? "SMC View: Reviews for Seth Baldwin" : isDad ? "Average Ratings by Member (All Reviews)" : isYouth ? `Reviews for ${selfMember?.name || "You"}` : "Average Ratings by Member"}
            </h2>
