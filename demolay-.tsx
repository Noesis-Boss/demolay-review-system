...
const getMemberInfo = (memberId: string) => members.find(m => m.id === memberId) || { name: memberId, title: "" };
...
export default function Results() {
  ...
  const exportCSV = () => {
    ...
    const rows = getDisplayReviews().map(r => [
      getMemberInfo(r.memberId).name, getMemberInfo(r.memberId).title, r.reviewerEmail || "Unknown",
  ...
  const getDisplayReviews = () => {
    ...
    console.log("User email:", userEmail, "isDad:", isDad, "viewAsSMC:", viewAsSMC);
    if (isDad && !viewAsSMC) {
  ...
  const getAveragesByMember = () => {
    ...
    console.log("Display reviews:", displayReviews.length);
    const grouped: any = {};
    displayReviews.forEach(r => {
      if (!grouped[r.memberId]) {
        const memberInfo = getMemberInfo(r.memberId);
        grouped[r.memberId] = { memberId: r.memberId, memberName: memberInfo.name, memberTitle: memberInfo.title, count: 0, ratings: { leadership: 0, teamwork: 0, attendance: 0, punctuality: 0, motivation: 0, ritualWork: 0, initiative: 0, planning: 0 } };
      }
