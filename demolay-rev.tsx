...
export default function Results() {
  ...
  const getDisplayReviews = () => {
    ...
    if (testMode && isDad) {
      // Test mode - show what SMC sees (only reviews about Seth Baldwin)
      ...
      <main className="max-w-6xl mx-auto px-4 py-8">
            ...
            {isDad && (
              <>
                <button 
                  ...
                  className="px-4 py-2 rounded-lg font-semibold text-white flex items-center gap-2 transition-all hover:opacity-90"
                  style={{ background: testMode ? AZ_RED : AZ_GOLD, color: testMode ? "white" : AZ_BLUE }}
                >
        ...
        {isDad && testMode && (
          <div className="mb-6 p-4 rounded-lg" style={{ background: `${AZ_GOLD}30`, border: `2px solid ${AZ_GOLD}` }}>
            <p className="font-semibold" style={{ color: AZ_BLUE }}>Test Mode: Viewing as SMC (Seth Baldwin)</p>
            <p className="text-sm" style={{ color: AZ_COPPER }}>Only showing reviews about Seth Baldwin</p>
          </div>
        )}
        ...
        {averages.length === 0 ? (
          ...
          <>
            <h2 className="text-xl font-semibold mb-4" style={{ color: AZ_BLUE }}>
              {testMode ? "Average Ratings for Seth Baldwin" : isDad ? "Average Ratings by Member (All Reviews)" : isYouth ? `Reviews for ${selfMember?.name || "You"}` : "Average Ratings by Member"}
            </h2>
