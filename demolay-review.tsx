  ...
  const loadReviewedMembers = async (token) => {
    ...
    for (const member of members) {
      try {
        ...
        const data = await res.json();
        if (data.reviewed) reviewed.push(member.id);
      } catch {}
    }
      ...
      <main className="max-w-6xl mx-auto px-4 py-8">
        <div className="mb-6 flex justify-between items-center flex-wrap gap-4">
          <div className="flex items-center gap-3">
            ...
            <div><p className="font-semibold" style={{ color: AZ_BLUE }}>{auth?.name}</p><p className="text-sm" style={{ color: "#B87333" }}>{auth?.email}</p></div>
          </div>
          <div className="flex gap-3">
            <Link to="/demolay-review/results" className="px-4 py-2 rounded-lg font-semibold text-white flex items-center gap-2 bg-green-600"><Eye className="w-4 h-4" />View Results</Link>
          ...
          {members.map((member) => {
            ...
            if (isReviewed) return (
              <div key={member.id} className="rounded-xl p-4 bg-green-50 border-2 border-green-500 shadow-lg">
                <div className="flex items-start justify-between mb-3">
                  <div className="w-12 h-12 rounded-full flex items-center justify-center bg-green-500"><CheckCircle2 className="w-6 h-6 text-white" /></div>
            ...
            return (
              <Link key={member.id} to={`/review/${member.id}`} className="rounded-xl p-4 bg-white shadow-lg transition-all hover:scale-105 hover:shadow-xl">
                ...
                <h3 className="font-semibold text-lg text-blue-900">{member.name}</h3>
                <p className="text-sm text-gray-600">{member.title}</p>
                <div className="flex items-center gap-1 text-sm text-yellow-600 mt-2"><Star className="w-4 h-4 fill-current" /><span>Click to review</span></div>
              </Link>
            );
          })}
        </div>
