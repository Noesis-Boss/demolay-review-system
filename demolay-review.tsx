...
export default function Landing() {
  ...
  // Check each member individually for reviewed status
  const loadReviewedMembers = async (token) => {
    const reviewed = [];
    for (const member of members) {
      try {
        const res = await fetch(`/api/reviews/submit?memberId=${member.id}`, { 
        ...
        console.error(`Failed to check ${member.id}:`, err);
      }
    }
      ...
      <main className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="mb-6 flex justify-between items-center flex-wrap gap-4">
          ...
          <div className="flex gap-3">
            <Link to="/demolay-review/results" className="px-4 py-2 rounded-lg font-semibold text-white flex items-center gap-2" style={{ background: "#16a34a" }}>
              <Eye className="w-4 h-4" />View Results
            </Link>
            <button onClick={handleLogout} className="px-4 py-2 rounded-lg font-semibold text-white" style={{ background: AZ_RED }}>
              Logout
            </button>
          </div>
        </div>
        ...
        {remainingCount === 0 ? (
          <div className="mb-8 p-6 rounded-xl text-center" style={{ background: "rgba(34,197,94,0.1)", border: "2px solid #22c55e" }}>
            ...
            <Link to="/demolay-review/results" className="inline-block px-6 py-3 rounded-lg font-semibold text-white" style={{ background: AZ_BLUE }}>
              View Results
            </Link>
          </div>
        ) : (
          ...
          {members.map((member) => {
            ...
            return (
              <Link 
                key={member.id} 
                to={`/review/${member.id}`} 
                className="rounded-xl p-4 bg-white shadow-lg transition-all hover:scale-105 hover:shadow-xl"
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  ...
                  <span className="px-2 py-1 rounded text-xs font-semibold" style={{ background: AZ_GOLD, color: AZ_BLUE }}>
                    PENDING
                  </span>
                </div>
                <h3 className="font-semibold text-lg" style={{ color: AZ_BLUE }}>{member.name}</h3>
