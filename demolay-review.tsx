...
import { Star, Lock, CheckCircle2, LogOut } from "lucide-react";
...
export default function Landing() {
  ...
  if (!auth || loading) return (
    <div className="min-h-screen flex items-center justify-center" style={{ background: LIGHT_BLUE_BG }}>
      <div className="text-center"><div className="w-12 h-12 border-4 border-t-transparent rounded-full animate-spin mx-auto mb-4" style={{ borderColor: AZ_GOLD }} /><p style={{ color: AZ_BLUE }}>Loading...</p></div>
    </div>
  );
      ...
      <main className="max-w-6xl mx-auto px-4 py-8">
        <div className="mb-6 flex justify-between items-center">
          ...
          <div className="flex gap-3">
            <Link to="/demolay-review/results" className="px-4 py-2 rounded-lg font-semibold text-white" style={{ background: AZ_BLUE }}>View All Results</Link>
            <button onClick={handleLogout} className="px-4 py-2 rounded-lg font-semibold text-white flex items-center gap-2" style={{ background: AZ_RED }}><LogOut className="w-4 h-4" />Logout</button>
          </div>
        </div>
        ...
        {selfMember && (
          <div className="mb-6 p-4 rounded-lg" style={{ background: `${AZ_RED}20`, borderLeft: `4px solid ${AZ_RED}` }}>
            <p className="font-semibold" style={{ color: AZ_RED }}>Self-review is disabled for your account</p>
            <p className="text-sm" style={{ color: AZ_COPPER }}>You cannot review yourself. Your results will be shown on the Results page.</p>
          </div>
        )}
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          {members.map((member) => {
            ...
            if (isSelf) return (
              <div key={member.id} className="rounded-xl p-4 opacity-60" style={{ background: "#e5e5e5", border: "2px dashed #999" }}>
                <div className="flex items-start justify-between mb-3">
                  ...
                  {isReviewed && <div className="p-1 rounded-full" style={{ background: "rgba(34,197,94,0.8)" }}><CheckCircle2 className="w-5 h-5 text-white" /></div>}
                </div>
                <h3 className="font-semibold text-lg" style={{ color: "#666" }}>{member.name}</h3>
                <p className="text-sm mb-3" style={{ color: "#888" }}>{member.title}</p>
                <div className="flex items-center gap-2 text-sm" style={{ color: "#888" }}><Lock className="w-4 h-4" /><span>Self-review disabled</span></div>
              </div>
            );
            return (
              <Link key={member.id} to={`/review/${member.id}`} className={`rounded-xl p-4 transition-all hover:scale-105 shadow-lg ${isReviewed ? "opacity-90" : ""}`} style={{ background: "white", border: isReviewed ? `2px solid ${AZ_GOLD}` : "2px solid transparent" }}>
                <div className="flex items-start justify-between mb-3">
                  <div className="w-12 h-12 rounded-full flex items-center justify-center font-bold text-lg" style={{ background: isReviewed ? "#fbbf24" : AZ_GOLD, color: AZ_BLUE }}>{member.name.split(" ").map(n => n[0]).join("").slice(0, 2)}</div>
                  {isReviewed && <div className="p-1 rounded-full" style={{ background: "rgba(34,197,94,0.8)" }}><CheckCircle2 className="w-5 h-5 text-white" /></div>}
                </div>
                ...
                <div className="flex items-center gap-1 text-sm" style={{ color: isReviewed ? "#16a34a" : AZ_GOLD }}><Star className="w-4 h-4 fill-current" /><span>{isReviewed ? "Reviewed" : "Click to review"}</span></div>
              </Link>
            );
          })}
        </div>
